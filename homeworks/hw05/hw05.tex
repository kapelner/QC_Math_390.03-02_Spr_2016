...
\title{MATH 390.03-02 / 650 Spring 2016 Homework \#5}
...
\hardsubproblem{What is the Jeffrey's prior for logit($\theta$) under the binomial likelihood?}\spc{2}
...
\intermediatesubproblem{Draw the picture from class and explain why the picture illustrates the Law of Total Variance for two r.v.'s $X$ and $Y$.}\spc{8}
...
\problem{We will now perform model checking by looking at pictures.}
...
\easysubproblem{If $\alpha = 100$ and $\beta = 3$ and $n=50$. Run the following code to look at the prior predictive distribution. No need to print out the illustration. You will need R on your computer for this (download it from \href{https://cran.r-project.org/bin/windows/base/}{here} if you are on windows and \href{https://cran.r-project.org/bin/macosx/}{here} if you are on MAC).

If you now get 10 successes on your 50 observations (the red line), does this indicate a problem in the model? Yes/No}
...
\end{verbatim}~\spc{0.1}

\easysubproblem{If you use the Jeffrey's prior $\alpha = 0.5$ and $\beta = 0.5$, run the following code to look at the prior predictive distribution. No need to print out the illustration.

If you now get 10 successes on your 50 observations, does this indicate a problem in the model? Yes/No}
...
\easysubproblem{If you use $\alpha = 100$ and $\beta = 3$ prior and you got 10 successes, we now look at the posterior predictive distribution by running the following code. No need to print out the illustration.

Does this indicate a problem in the model? Yes/No}
...
\easysubproblem{If you use the Jeffrey's prior with the same data, we now look at the posterior predictive distribution by running the following code. No need to print out the illustration.
...
\easysubproblem{Why should the data look plausible under the posterior predictive distribution?}\spc{3}

\easysubproblem{Why are looking at these illustrations called \qu{graphical checks}?}\spc{3}

\easysubproblem{Why are looking at these illustrations called \qu{self-consistency checks}?}\spc{3}

\hardsubproblem{[MA] \href{https://en.wikipedia.org/wiki/George_E._P._Box}{George Box} said, \qu{all models are wrong but some are useful}. What does this mean?}\spc{3}

\hardsubproblem{[MA] \href{https://en.wikipedia.org/wiki/Andrew_Gelman}{Andrew Gelman} refines George Box's quotation by saying, we know all models are wrong \qu{but do the model's deficiencies have a noticable impact on inference}? What does he mean by asking this question? Keep in mind Gelman loves these graphical checks a la (b), (c), (e) and (f).}\spc{5}
