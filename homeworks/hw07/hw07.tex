...
\date{Due in my mail slot, Friday, April 8, 2016 \\ \vspace{0.5cm} \small (this document last updated \today ~at \currenttime)}
}
...
\easysubproblem{If $\Xoneton \iid \normnot{\theta}{\sigsq}$ and $\Xbar := \oneover{n}\sum_{i=1}^n X_i$, what is the distribution of $\Xbar$ assuming $\sigsq$ is known? This is from your Math 241 notes.}\spc{1}
...
\hardsubproblem{The company wishes to test if there's too much variability i.e. that there is more than $\sigma = 0.1$ variability. You take a sample of 10 and see 1.153, 1.045, 1.268, 1.333, 0.799, 1.075, 1.27, 1.07, 1.192 and 1.079 quarts. Find the $p$ value. You can write the answer below as a function of \texttt{rinvgamma} or \texttt{pinvgamma}. E.C. for computing it and testing this at $\alpha = 5\%$. You may want to use the \texttt{actuar} package (see \href{http://www.inside-r.org/packages/cran/actuar/docs/pinvgamma}{here}).}\spc{12}

\end{enumerate}

\problem{Now we will move to the Bayesian normal-normal model for estimating $\sigsq$ and demonstrate the similarities with the classical results.}

\begin{enumerate}

\easysubproblem{If $\Xoneton \exchdist \normnot{\theta}{\sigsq}$ and $X$ represents all $\Xoneton$, in HW6 6(c) you found the kernel of $\theta,~\sigsq~|~X = x$. Do so again below but this time use the substition that we made in class:

\beqn
\sum_{i=1}^n (x_i - \theta)^2 = (n-1)s^2 + n(\xbar - \theta)^2
\eeqn

where $s^2$ is your answer from 2(e). We do this here because this substition is important for what comes next.}\spc{10}

\intermediatesubproblem{If $\prob{\theta,~\sigsq} \propto \oneover{\sigsq}$, show that this is a conjugate prior for the posterior of both the mean and variance, $\cprob{\theta,~\sigsq}{X}$. We called this two-dimensional distribution the normal-inverse-gamma but we did not go into details about it.}\spc{4}
...
\easysubproblem{If $\Xoneton \exchdist \normnot{\theta}{\sigsq}$ and $\theta \sim \normnot{\mu_0}{\tausq}$ write the distribution of $\theta~|~X,\sigsq$. Hint: it's in the notes and it was HW6 6(d).}\spc{5}

\easysubproblem{Find $\cprob{\theta}{X,~\sigsq}$ which is the first part of (c) above. This is in the notes. Note how this prior is equivalent to saying $\prob{\theta} \propto 1$.}\spc{3}
...
\easysubproblem{Explain how to sample from the distribution of $X^*~|~X$.}\spc{4}
...
\intermediatesubproblem{Regardless of normality, do you think the data comes from exchangeable r.v.'s?}\spc{4}

\intermediatesubproblem{Assume exchangeability and normality regardless of what you wrote in (a) and (b). The sample average is $\xbar=$0.0003415 and the sample standard deviation is $s=$0.0096. Under a relatively objective prior, give a 95\% credible region for the true daily return.}\spc{4}

\intermediatesubproblem{Give a 95\% credible region for tomorrow's return.}\spc{15}

\intermediatesubproblem{Below is the posterior predictive distribution in blue graphed atop the actual data in red. Is our model a good fit? Yes/no and explain.}\spc{4}
