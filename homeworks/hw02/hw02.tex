...
\hardsubproblem{[MA] Find the MLE for $\theta$ if $\Xoneton \iid \normnot{\mu}{\sigsq}$. Solve the system of equations $\partialop{\mu}{\ell(\theta)} = 0$ and $\partialop{\sigsq}{\ell(\theta)} = 0$ where $\ell(\theta)$ denotes the log likelihood. You can easily find this online. But try to do it yourself.} \spc{8}
...
\easysubproblem{Given parts (a) and (b), what does everyone need to start with for conclusions to be the same?}\spc{1}

\extracreditsubproblem{[MA] For some reason in order for the logical definition to work, Keynes required the principle of indifference which is a stronger assumption than everyone begins with equal priors. I can't figure out why. Extra credit to anyone who figures out why this is the case.}\spc{0}


\easysubproblem{In order for the logical definition to work, one must employ the principle of indifference. State that principle for a discrete set $\Theta_0 = \braces{\theta_1, \ldots, \theta_m}$ by giving the prior distribution of $\theta$.}\spc{2}

\easysubproblem{In order for the logical definition to work, one must employ the principle of indifference. State that principle for a continuous set $\Theta_0 = \bracks{a,~b}$ by giving the prior distribution of $\theta$.}\spc{2}
...
\intermediatesubproblem{The denominator of the expression for the posterior $\prob{\Xoneton}$ is called the \qu{marginal likelihood} or the \qu{prior on the data}. Explain why if the \qu{classic} idea of independence you learned in 241 i.e. $\prob{\Xoneton} = \prod_{i=1}^n \prob{X_i}$ was applicable in the Bayesian perspective then no learning can be done from experience. Use the expression for the posterior predictive distribution to make a simple one-line statement.}\spc{3}

\intermediatesubproblem{[MA] We have showed that $\Xoneton$ are not independent. Show here that although they are not independent, they are identically distributed. You will need to use the property that $X_1~|~\theta, \ldots, X_n~|~\theta \iid p(x)$. I know this is splitting hairs, but it's a good property to prove.}\spc{3}

\hardsubproblem{We showed in class that $\Xoneton$ are not independent. Independence is in some sense \qu{too strong} in te }\spc{3}
