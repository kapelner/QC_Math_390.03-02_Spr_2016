...
\easysubproblem{The main attack on Bayesian Statistics has always been subjectivity. Answer the following question how Savage would have answered it: \qu{If prior opinions can differ from one researcher to the next, what happens to scientifit objectivity in data analysis?} Do you believe this is the way science works in the real world?}\spc{3}


\hardsubproblem{[MA] On page 104, Sharon writes, \qu{Bayesians would also be able to concentrate on what happened, not on what \textit{could} have happened according to Neyman Pearson's samping plan}. (Note that the \qu{Neyman Pearson's samping plan} is synonymous with Frequentist Statistics). Explain (1) how Bayesians concentrate on \qu{what happened} and (2) how Frequentists concentrate on what \qu{\textit{could} have happened}.}\spc{6}
...
\intermediatesubproblem{Given the same prior as previously, imagine the surface area which is calculated as $6S^2$. Find expectation of the surface area. You may need to look at your Math 241 notes to get the variance of the uniform r.v. Is this equal to $6 \expe{S}^2$?}\spc{6}
...
\easysubproblem{Assuming that you wrote \qu{yes} to the previous problem, calculate 

\beqn
\prob{\inverse{\text{wine/water}} \geq 2^{-1}} = \prob{\text{water/wine} \leq \half}.
\eeqn

And: is this different to the answer you got in (e)?} \spc{4}
