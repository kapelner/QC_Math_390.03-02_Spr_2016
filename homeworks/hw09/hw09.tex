...
\intermediatesubproblem{According to Wikipedia, there were 895 males and 980 females? Using that information and your answer to (a) and (b) and the information given in (c), construct $\prob{\theta}$. This is review for the final as a question similar to this one was on the midterm.}
...
We need to compute $\rho_M' = \rho_M \frac{\probsub{M}{X}}{\probsub{M}{X} + \probsub{F}{X}}$ where:
...
&=&  \tothepow{\oneoversqrt{2\pi \sigsq}}{n} \int_\reals  \exp{-\oneover{2\sigsq} \underbrace{\sum_{i=1}^n \squared{x_i - \theta}}_{\text{Lec 13 page 5}}}  \normnot{\mu_M}{\tausq_M}d\theta \\
...
&=&  \tothepow{\oneoversqrt{2\pi \sigsq}}{n} e^{-\oneover{2\sigsq} \parens{(n-1)s^2}} \sqrt{2\pi\sigsq/n} \underbrace{\int_\reals  \normnot{\xbar}{\sigsq/n}  \normnot{\mu_M}{\tausq_M}d\theta}_{\text{See p9 Lec 12}} \\
&=&  \tothepow{\oneoversqrt{2\pi \sigsq}}{n} e^{-\oneover{2\sigsq} \parens{(n-1)s^2}} \sqrt{2\pi\sigsq/n}   ~\underbrace{\normnot{\mu_M}{\sigsq/n + \tausq_M}}_{\text{free variable: $\xbar$}} \\
\eeqn
